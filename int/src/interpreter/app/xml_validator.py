...
from ..input_model import Parameter as AstParameter
...
SPECIAL_VARIABLE_NAMES = {"self", "super", "nil", "true", "false"}
...
def _validate_identifier_tail(identifier: str, what: str) -> None:
    """
    @brief Remaining identifier characters are validated.
...
def _validate_identifier_lexeme(identifier: str, what: str) -> None:
    ...
    _validate_identifier_tail(identifier, what)
...
def _validate_selector_keyword_parts(selector: str) -> None:
    """
    @brief Keyword parts of one keyword selector are validated.
...
def _validate_selector_lexeme(selector: str) -> None:
    ...
    _validate_selector_keyword_parts(selector)
...
def _validate_assign_target_lexeme(target_name: str) -> None:
    ...
    """
    if target_name in SPECIAL_VARIABLE_NAMES:
        return

    _validate_identifier_lexeme(target_name, "variable name")


def _validate_integer_literal_value(value: str) -> None:
    """
    @brief One integer literal value is validated.

    @param value An integer literal value to be inspected.
    """
    if not value:
        raise InterpreterError(
            ErrorCode.INT_STRUCTURE,
    ...
    if value[0] in {"+", "-"}:
        if len(value) == 1:
            raise InterpreterError(
                ErrorCode.INT_STRUCTURE,
                f"Invalid integer literal value: {value}",
            )
        start_index = 1
...
def _validate_nil_literal_value(value: str) -> None:
    """
    @brief One Nil literal value is validated.

    @param value A literal value to be inspected.
    """
    if value != "nil":
        raise InterpreterError(
            ErrorCode.INT_STRUCTURE,
            f"Invalid Nil literal value: {value}",
        )


def _validate_true_literal_value(value: str) -> None:
    """
    @brief One True literal value is validated.
    ...
    if value != "true":
        raise InterpreterError(
            ErrorCode.INT_STRUCTURE,
            f"Invalid True literal value: {value}",
        )


def _validate_false_literal_value(value: str) -> None:
    """
    @brief One False literal value is validated.
    ...
    if value != "false":
        raise InterpreterError(
            ErrorCode.INT_STRUCTURE,
            f"Invalid False literal value: {value}",
        )


def _validate_class_literal_value(value: str) -> None:
    """
    @brief One class literal value is validated.
...
def _validate_literal_xml_ast(literal: AstLiteral) -> None:
    ...
    if literal.class_id == "Integer":
        _validate_integer_literal_value(literal.value)
        return

    if literal.class_id == "Nil":
        _validate_nil_literal_value(literal.value)
        return

    if literal.class_id == "True":
        _validate_true_literal_value(literal.value)
        return

    if literal.class_id == "False":
        _validate_false_literal_value(literal.value)
        return

    if literal.class_id == "class":
        _validate_class_literal_value(literal.value)
        return
...
def _validate_var_expr_xml_ast(var: AstVar) -> None:
    """
    @brief One variable expression branch is validated.

    @param var A variable AST node to be inspected.
    """
    if var.name in SPECIAL_VARIABLE_NAMES:
        return

    _validate_identifier_lexeme(var.name, "variable name")


def _validate_literal_expr_xml_ast(literal: AstLiteral) -> None:
    """
    @brief One literal expression branch is validated.

    @param literal A literal AST node to be inspected.
    """
    _validate_literal_xml_ast(literal)


def _validate_assign_target_xml_ast(target: AstVar) -> None:
    """
    @brief One assignment target AST node is validated.

    @param target An assignment target variable AST node to be inspected.
    """
    _validate_assign_target_lexeme(target.name)


def _validate_parameter_xml_ast(parameter: AstParameter) -> None:
    """
    @brief One parameter AST node is validated for additional XML-structure rules.

    @param parameter A parameter AST node to be inspected.
    """
...
class XmlValidator:
    ...
    def validate_program_ast(self, program: Program) -> None:
        ...
        for class_def in program.classes:
            self._validate_class_xml_ast(class_def)

    def _validate_class_xml_ast(self, class_def: AstClassDef) -> None:
        """
        @brief One class AST node is validated for additional XML-structure rules.

        @param class_def A class AST node to be inspected.
        """
        ...
        for method in class_def.methods:
            self._validate_method_xml_ast(method)

    def _validate_method_xml_ast(self, method: AstMethod) -> None:
        """
        @brief One method AST node is validated for additional XML-structure rules.

        @param method A method AST node to be inspected.
        """
        _validate_selector_lexeme(method.selector)
        self._validate_block_xml_ast(method.block)

    def _validate_block_xml_ast(self, block: AstBlock) -> None:
        """
        ...
        for parameter in block.parameters:
            _validate_parameter_xml_ast(parameter)
    ...
    def _validate_assign_xml_ast(self, assign: AstAssign) -> None:
        """
        @brief One assignment AST node is validated for additional XML-structure rules.

        @param assign An assignment AST node to be inspected.
        """
        _validate_assign_target_xml_ast(assign.target)
        self._validate_expr_xml_ast(assign.expr)

    def _validate_expr_xml_ast(self, expr: AstExpr) -> None:
        """
        ...
        if expr.literal is not None:
            _validate_literal_expr_xml_ast(expr.literal)
            return

        if expr.var is not None:
            _validate_var_expr_xml_ast(expr.var)
            return

        if expr.block is not None:
            self._validate_block_expr_xml_ast(expr.block)
            return

        if expr.send is not None:
            self._validate_send_expr_xml_ast(expr.send)
            return

    def _validate_block_expr_xml_ast(self, block: AstBlock) -> None:
        """
        @brief One block expression branch is validated.

        @param block A block AST node to be inspected.
        """
        self._validate_block_xml_ast(block)

    def _validate_send_expr_xml_ast(self, send: AstSend) -> None:
        """
        @brief One send expression branch is validated.

        @param send A send AST node to be inspected.
        """
        self._validate_send_xml_ast(send)
